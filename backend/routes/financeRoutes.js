...
router.post('/process-payment', async (req, res) => {
  const { invoiceId, paymentAmount, paymentMethod, paymentDate = new Date().toISOString().split('-')[0] } = req.body;
  ...
  try {
    ...
    let newInvoiceStatus = invoice.status; // 注意：這裡將發票狀態變數名稱從 newStatus 改為 newInvoiceStatus，以免與付款狀態混淆
...
router.get('/top-customers-by-revenue', async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute(`
      ...
      ORDER BY total_revenue DESC
      LIMIT ${limit};
    `); // Changed to template literal for LIMIT
    res.json(rows);
  } catch (error) {
